...
class IRGenerator implements ASTVisitor<Void, Expr> {
    ...
    private c.asm.Type asmType(Type t) {
        if (t.isVoid()) return int_t();
        return c.asm.Type.get(t.size());
    }

    private c.asm.Type varType(Type t) {
        if (! t.isScalar()) {
            return null;
        }
        return c.asm.Type.get(t.size());
    ...
    private c.asm.Type int_t() {
        return c.asm.Type.get((int)typeTable.intSize());
    }

    private c.asm.Type size_t() {
        return c.asm.Type.get((int)typeTable.longSize());
    }

    private c.asm.Type ptr_t() {
        return c.asm.Type.get((int)typeTable.pointerSize());
    }

    private c.asm.Type ptrdiff_t() {
        return c.asm.Type.get((int)typeTable.longSize());
    }
